sqlConfidenceApp.controller('ExerciseCtrl', function ($scope, $http, $filter, $timeout, boostrappedExerciseData, bootstrappedUserData) {
    ...
    $scope.RenderTableFromResponse = function(response)
    {
        ...
        angular.forEach($scope.rows, function (row) {
            ...
            {
                var value = row[prop];
                if (value.substring !== undefined && value.substring(0, 5) === '/Date') {
