...
namespace SqlConfidence.Helpers
{
    public class AdminRedirectAttribute : ActionFilterAttribute
    {
        public override void OnActionExecuting(ActionExecutingContext filterContext)
        {
            ...
            method = typeof(BaseController).GetMethod("GetActiveUser");
            User user = (User)method.Invoke(filterContext.Controller, null);
            if (!user.IsAdmin) throw new Exception("User must be an administrator to access the admin console - " + user.Email);
        }
    }
}