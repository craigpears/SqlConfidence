...
namespace Common
{
    public class DataAccess
    {
        ...
        #region GetMethods
            ...
            public void ResetQuestion(int UserId, int QuestionId)
            {
                try
                {
                    ...
                    {
                        conn.Open();
                        using (SqlCommand command = new SqlCommand("DELETE FROM USERS_ACTIONS WHERE US_ID = @UserId AND EXQ_ID = @QuestionId", conn))
                        {
                            command.Parameters.AddWithValue("UserId", UserId);
                            command.Parameters.AddWithValue("QuestionId", QuestionId);
        ...
        public void LogUserQuery(int UserId, string Query, int QuestionId, UsersActionsTypes QueryType)
        {
            ...
            {
                conn.Open();
                using (SqlCommand command = new SqlCommand("insert into USERS_ACTIONS (US_ID, USA_DESCRIPTION, EXQ_ID, USAT_ID, USA_CREATED_DATE) VALUES (@UserId, @Query, @QuestionId, @ActionTypeId, GETDATE())", conn))
                {
                    command.Parameters.AddWithValue("UserId", UserId);
        ...
        {           
            try
            {
                ...
                {
                    conn.Open();
                    using (SqlCommand command = new SqlCommand("select cast(case when exists(select * from exercises_questions_answered where us_id = @US_ID and exq_id = @EXQ_ID) then 1 else 0 end as bit) 'exists'", conn))
                    {
                        command.Parameters.AddWithValue("US_ID", UserId);
                        command.Parameters.AddWithValue("EXQ_ID", QuestionId);
                    ...
                    if (!exists)
                    {
                        using (SqlCommand command = new SqlCommand("insert into EXERCISES_QUESTIONS_ANSWERED (US_ID, EXQA_CREATED_DATE, EXQ_ID) VALUES (@UserId, GETDATE(), @QuestionId)", conn))
                        {
                            command.Parameters.AddWithValue("UserId", UserId);
                            command.Parameters.AddWithValue("QuestionId", QuestionId);
                            command.ExecuteNonQuery();
        ...
        public void SaveQuestion(QuestionModel question, UserModel user, Boolean LogUpdatedBy = true)
        {
            // Is this a new exercise?
        ...
        public void PublishExercise(int ExerciseId, UserModel user)
        {
            using (SqlConnection conn = new SqlConnection(Properties.Settings.Default.DatabaseConnectionString))
            {
                conn.Open();
